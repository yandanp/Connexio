...
Token status tidak dipetakan ke palette Tailwind `connexio-*` — pakai via `var(--success-color)`,
`var(--warning-color)`, `var(--danger-color)` di CSS/inline style.
...
## 3. Kelas utilitas — 11 kelas inti (+ varian)
