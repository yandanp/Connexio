...
function CodeBlock({ code, lang, onRun, onInsert }: { code: string; lang: string; onRun: (cmd: string) => void; onInsert: (code: string) => void }) {
	...
	return <div className="my-2 overflow-hidden rounded-lg bg-connexio-bg shadow-[inset_0_0_0_1px_rgba(255,255,255,0.05)]">
		<div className="flex items-center justify-between bg-connexio-bg-tertiary px-2 py-1 shadow-[inset_0_-1px_0_rgba(255,255,255,0.05)]">
			...
			<div className="flex items-center gap-1">
				{shell && <button onClick={() => onRun(code)} className="flex items-center gap-1 px-1.5 py-0.5 dock-button text-[10px] text-green-400" type="button"><Play size={10} />Run</button>}
				<button onClick={() => onInsert(code)} className="flex items-center gap-1 px-1.5 py-0.5 dock-button text-[10px] text-connexio-accent" type="button"><FileText size={10} />Insert</button>
				<button onClick={copy} className="flex items-center gap-1 px-1.5 py-0.5 dock-button text-[10px] text-connexio-text-muted" type="button">{copied ? <Check size={10} className="text-green-400" /> : <Copy size={10} />}{copied ? "Copied" : "Copy"}</button>
			</div>
		</div>
...
function MessageBubble({ message, onRun, onInsert }: { message: AIMessage; onRun: (cmd: string) => void; onInsert: (code: string) => void }) {
	...
	return <div className={`flex gap-2 px-3 py-2.5 ${isUser ? "" : "bg-connexio-bg-secondary/45"}`}>
		<div className="flex-shrink-0 mt-0.5">{isUser ? <div className="flex h-6 w-6 items-center justify-center rounded-lg bg-connexio-accent/15 shadow-[inset_2px_0_0_var(--accent-color)]"><span className="text-[9px] font-bold text-connexio-accent">U</span></div> : <div className="flex h-6 w-6 items-center justify-center rounded-lg bg-connexio-accent/10"><Bot size={11} className="text-blue-400" /></div>}</div>
		<div className="flex-1 min-w-0">
			{message.context && <div className="flex gap-1 mb-1 text-[9px] text-connexio-text-muted">{message.context.file && <span className="px-1 field-soft">file context</span>}{message.context.terminal && <span className="px-1 field-soft">terminal context</span>}</div>}
			...
			{message.content && !message.isStreaming && <button onClick={copy} className="mt-1 flex items-center gap-1 p-0.5 dock-button text-[10px] text-connexio-text-muted" type="button">{copied ? <Check size={10} className="text-green-400" /> : <Copy size={10} />}{copied ? "Copied" : "Copy"}</button>}
		</div>
	</div>;
}
...
export default function AIChatPanel() {
	...
	return <div className="flex h-full flex-col bg-connexio-bg-secondary/35">
		<div className="flex items-center justify-between px-3 py-2.5 shadow-[inset_0_-1px_0_rgba(255,255,255,0.05)]">
			<div className="flex items-center gap-1.5 min-w-0"><Bot size={12} className="text-connexio-accent flex-shrink-0" /><span className="text-[10px] font-semibold uppercase tracking-wider text-connexio-text-muted">AI Chat</span><select value={config.activeProviderId} onChange={(e) => setActiveProvider(e.target.value)} className="max-w-[90px] text-[9px] px-1 py-0.5 field-soft text-connexio-text outline-none">{config.providers.filter((p) => p.enabled).map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><select value={config.activeModel} onChange={(e) => setActiveProvider(config.activeProviderId, e.target.value)} className="max-w-[110px] text-[9px] px-1 py-0.5 field-soft text-connexio-text outline-none">{(activeProvider?.models || [config.activeModel]).map((m) => <option key={m} value={m}>{m.split("/").pop()}</option>)}</select></div>
			<div className="flex items-center gap-0.5"><button onClick={() => setShowHistory(!showHistory)} className="p-1 dock-button" title="History" type="button"><History size={11} className="text-connexio-text-muted" /></button><button onClick={newChat} className="p-1 dock-button" title="New chat" type="button"><Plus size={11} className="text-connexio-text-muted" /></button><button onClick={clearMessages} className="p-1 dock-button" title="Clear" type="button"><Trash2 size={11} className="text-connexio-text-muted" /></button><button onClick={() => setShowSettings(true)} className="p-1 dock-button" title="Settings" type="button"><Settings size={11} className="text-connexio-text-muted" /></button></div>
		</div>
		{showHistory && <div className="max-h-36 overflow-y-auto bg-connexio-bg-secondary/70 shadow-[inset_0_-1px_0_rgba(255,255,255,0.05)]">{sessions.length === 0 ? <div className="px-3 py-2 text-[11px] text-connexio-text-muted">No saved chats</div> : sessions.slice().reverse().map((s) => <div key={s.id} className="flex items-center gap-1 px-2 py-1 hover:bg-connexio-bg-tertiary"><button onClick={() => { loadChatSession(s.id); setShowHistory(false); }} className="flex-1 text-left text-[11px] text-connexio-text truncate" type="button">{s.title}</button><button onClick={() => deleteChatSession(s.id)} className="p-0.5 rounded hover:bg-red-500/10" type="button"><X size={10} className="text-red-400" /></button></div>)}</div>}
		<div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden">{messages.length === 0 ? <div className="flex h-full flex-col items-center justify-center px-5 text-center"><div className="mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-connexio-accent/10 text-connexio-accent shadow-[inset_2px_0_0_var(--accent-color)]"><Bot size={24} /></div><p className="text-[12px] font-semibold text-connexio-text">Ask about this workspace</p><p className="mt-1 max-w-[260px] text-[11px] leading-5 text-connexio-text-muted">Use active file or terminal context to debug errors, explain code, or generate commands.</p><div className="mt-4 grid w-full max-w-[280px] gap-2"><button onClick={() => setInput('Explain the active file and suggest improvements')} className="rounded-xl bg-white/[0.035] px-3 py-2 text-left text-[10px] text-connexio-text-secondary transition-colors hover:bg-white/[0.055]" type="button">Explain active file</button><button onClick={() => setInput('Help me debug the latest terminal error')} className="rounded-xl bg-white/[0.035] px-3 py-2 text-left text-[10px] text-connexio-text-secondary transition-colors hover:bg-white/[0.055]" type="button">Debug terminal error</button></div></div> : <>{messages.map((msg) => <MessageBubble key={msg.id} message={msg} onRun={runCommand} onInsert={insertCode} />)}<div ref={endRef} /></>}</div>
		<div className="p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]"><div className="flex items-center gap-2 mb-1.5 text-[10px] text-connexio-text-muted"><label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={includeFile} onChange={(e) => setIncludeFile(e.target.checked)} className="w-3 h-3" />Active file</label><label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={includeTerminal} onChange={(e) => setIncludeTerminal(e.target.checked)} className="w-3 h-3" />Terminal</label></div><div className="flex items-end gap-1.5 field-soft px-2 py-1.5"><textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder="Ask AI..." rows={1} className="flex-1 bg-transparent text-[12px] text-connexio-text placeholder:text-connexio-text-muted outline-none resize-none max-h-[100px]" />{isLoading ? <button onClick={stopStreaming} className="p-1 rounded hover:bg-red-500/20" type="button"><Square size={12} className="text-red-400" /></button> : <button onClick={send} disabled={!input.trim()} className="p-1 rounded hover:bg-connexio-accent/20 disabled:opacity-30" type="button"><Send size={12} className="text-connexio-accent" /></button>}</div></div>
	</div>;
}
...
function AISettingsPanel({ onBack }: { onBack: () => void }) {
	...
	return <div className="flex h-full flex-col bg-connexio-bg-secondary/35"><div className="flex items-center gap-2 px-3 py-2 border-b border-connexio-border"><button onClick={onBack} className="text-[11px] text-connexio-accent hover:underline" type="button">← Back</button><span className="text-[10px] font-semibold uppercase tracking-wider text-connexio-text-muted">AI Settings</span></div><div className="flex-1 overflow-y-auto p-3 space-y-4">
		<div><div className="flex items-center justify-between mb-1"><label className="text-[10px] font-medium text-connexio-text-secondary">Providers</label><button onClick={addCustom} className="flex items-center gap-1 text-[10px] text-connexio-accent hover:underline" type="button"><Plus size={10} />Add</button></div><select value={selectedId} onChange={(e) => setSelectedId(e.target.value)} className="w-full text-[11px] px-2 py-1.5 field-soft text-connexio-text outline-none">{config.providers.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
		{selected && <div className="space-y-3 rounded-xl bg-connexio-bg-secondary/60 p-2"><div><label className="text-[10px] text-connexio-text-secondary">Name</label><input value={selected.name} onChange={(e) => update({ name: e.target.value })} className="w-full mt-1 text-[11px] px-2 py-1.5 field-soft text-connexio-text outline-none" /></div><div><label className="text-[10px] text-connexio-text-secondary">Type</label><select value={selected.type} onChange={(e) => update({ type: e.target.value as AIProviderType })} className="w-full mt-1 text-[11px] px-2 py-1.5 field-soft text-connexio-text outline-none">{providerTypes.map((t) => <option key={t} value={t}>{t}</option>)}</select></div><div><label className="text-[10px] text-connexio-text-secondary">API Key</label><input type="password" value={selected.apiKey} onChange={(e) => update({ apiKey: e.target.value })} placeholder="sk-..." className="w-full mt-1 text-[11px] px-2 py-1.5 field-soft text-connexio-text outline-none" /></div><div><label className="text-[10px] text-connexio-text-secondary">Base URL</label><input value={selected.baseUrl || ""} onChange={(e) => update({ baseUrl: e.target.value })} placeholder="Optional / local endpoint" className="w-full mt-1 text-[11px] px-2 py-1.5 field-soft text-connexio-text outline-none" /></div><div><label className="text-[10px] text-connexio-text-secondary">Models</label><div className="mt-1 space-y-1">{selected.models.map((m) => <div key={m} className="flex items-center gap-1"><button onClick={() => { update({ defaultModel: m }); if (config.activeProviderId === selected.id) setActiveProvider(selected.id, m); }} className={`flex-1 text-left text-[11px] px-2 py-1 rounded ${selected.defaultModel === m ? "bg-connexio-accent/10 text-connexio-accent" : "bg-connexio-bg-tertiary text-connexio-text"}`} type="button">{m}</button><button onClick={() => update({ models: selected.models.filter((x) => x !== m) })} type="button"><X size={10} className="text-red-400" /></button></div>)}</div><div className="flex gap-1 mt-1"><input value={modelInput} onChange={(e) => setModelInput(e.target.value)} placeholder="model-name" className="flex-1 text-[11px] px-2 py-1 field-soft text-connexio-text outline-none" /><button onClick={() => { if (modelInput.trim()) { update({ models: [...selected.models, modelInput.trim()], defaultModel: selected.defaultModel || modelInput.trim() }); setModelInput(""); } }} className="px-2 text-[10px] rounded bg-connexio-accent/10 text-connexio-accent" type="button">Add</button></div></div><label className="flex items-center gap-2 text-[11px] text-connexio-text-secondary"><input type="checkbox" checked={selected.enabled} onChange={(e) => update({ enabled: e.target.checked })} />Enabled</label><button onClick={() => { setActiveProvider(selected.id); onBack(); }} className="w-full flex items-center justify-center gap-1 px-2 py-1.5 text-[11px] rounded bg-connexio-accent/10 text-connexio-accent hover:bg-connexio-accent/20" type="button"><Save size={10} />Use This Provider</button>{selected.id.startsWith("custom-") && <button onClick={() => { removeProvider(selected.id); setSelectedId(config.providers[0]?.id || ""); }} className="w-full px-2 py-1 text-[11px] rounded text-red-400 hover:bg-red-500/10" type="button">Delete Provider</button>}</div>}
		<div><label className="text-[10px] font-medium text-connexio-text-secondary mb-1 block">System Prompt</label><textarea value={config.systemPrompt} onChange={(e) => setConfig({ systemPrompt: e.target.value })} rows={5} className="w-full text-[11px] px-2 py-1.5 field-soft text-connexio-text outline-none resize-none" /></div><label className="flex items-center gap-2 text-[11px] text-connexio-text-secondary"><input type="checkbox" checked={config.streamingEnabled} onChange={(e) => setConfig({ streamingEnabled: e.target.checked })} />Enable streaming response</label>
	</div></div>;
}
