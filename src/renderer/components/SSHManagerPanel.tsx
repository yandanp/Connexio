...
export default function SSHManagerPanel({ projectId, onConnect, onOpenSftp, initialView = "hosts" }: Props) {
	...
	const [hostMenu, setHostMenu] = useState<{ x: number; y: number; conn: SSHConnection; scope: "project" | "global" } | null>(null);
	const [deleteConfirm, setDeleteConfirm] = useState<{ id: string; name: string; scope: "project" | "global" } | null>(null);
			...
			{connectPrompt && (
				<div className="m-2 p-2 rounded border border-connexio-accent/40 bg-connexio-bg-tertiary space-y-2">
					...
					<div className="flex gap-1">
						<button onClick={handleConnectWithPassword} className="px-2 py-1 text-[10px] rounded bg-connexio-accent text-connexio-bg" type="button">Connect</button>
			...
			{activeView === "hosts" && (
				<div className="p-6">
					<div className="flex items-center gap-2 mb-7">
						<button onClick={() => setIsAdding("project")} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-connexio-bg-secondary text-connexio-text border border-connexio-border  hover:border-connexio-accent/50" type="button">
							<Plus size={15} />
							<span className="text-sm font-semibold">New host</span>
						</button>
						<button onClick={() => setIsAdding("global")} className="flex items-center gap-2 px-4 py-2 rounded-xl text-connexio-text-muted hover:bg-connexio-bg-secondary/70" type="button">
							<Plus size={13} />
					...
					<div className="flex items-center justify-between mb-4">
						...
						{connectStatus && <div className="text-xs text-connexio-accent animate-pulse">{connectStatus}</div>}
					</div>
					{isAdding === "project" && (
						<div className="mb-4 max-w-2xl">
							<div className="text-[10px] text-connexio-text-muted mb-1">Adding to this project</div>
							<SSHEditForm section={editSection} onSectionChange={setEditSection} onSave={(c) => handleSave(c, "project")} onCancel={() => setIsAdding(null)} />
						</div>
					)}
					{isAdding === "global" && (
						<div className="mb-4 max-w-2xl">
							<div className="text-[10px] text-connexio-text-muted mb-1">Adding as global (available in all projects)</div>
							<SSHEditForm section={editSection} onSectionChange={setEditSection} onSave={(c) => handleSave(c, "global")} onCancel={() => setIsAdding(null)} />
						</div>
					)}
					<div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-3">
						{filteredConnections.map((conn) => editingId === conn.id ? (
							<div key={conn.id} className="rounded-2xl bg-connexio-bg-secondary border border-connexio-border p-3">
								<SSHEditForm connection={conn} section={editSection} onSectionChange={setEditSection} onSave={(c) => handleSave(c, connections.some((item) => item.id === conn.id) ? "project" : "global")} onCancel={() => setEditingId(null)} />
							</div>
						) : (
							<SSHHostCard
								key={conn.id}
								...
								onDelete={() => handleDelete(conn.id, connections.some((item) => item.id === conn.id) ? "project" : "global")}
								onContextMenu={(e: React.MouseEvent) => { e.preventDefault(); setHostMenu({ x: e.clientX, y: e.clientY, conn, scope: connections.some((item) => item.id === conn.id) ? "project" : "global" }); }}
							/>
						))}
					</div>
					{filteredConnections.length === 0 && <div className="text-sm text-connexio-text-muted">No hosts found.</div>}
				</div>
			)}
			...
			{hostMenu && (
				<SSHManagerContextMenu
					...
					items={[
						...
						{ label: "Duplicate", onClick: () => handleSave({ ...hostMenu.conn, id: uuid(), name: `${hostMenu.conn.name} Copy` }, hostMenu.scope) },
						{ label: "Delete", danger: true, onClick: () => handleDelete(hostMenu.conn.id, hostMenu.scope) },
					]}
				/>
			)}
...
function SSHEditForm({
	...
	const handleSave = async () => {
		if (!name.trim() || !host.trim() || !username.trim()) {
			const missing = [!name.trim() && "name", !host.trim() && "host", !username.trim() && "username"].filter(Boolean);
		...
		<div className="p-4 space-y-4 bg-connexio-bg-tertiary rounded-xl border border-connexio-border">
			...
			{section === "basic" && (
				<div className="space-y-3">
					<div>
						<label className="block text-[11px] font-medium text-connexio-text-secondary mb-1">Connection name</label>
						<input ref={nameRef} type="text" placeholder="e.g. Production Server" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={handleKeyDown} className="w-full px-3 py-2 text-xs bg-connexio-bg border border-connexio-border rounded-lg text-connexio-text outline-none focus:border-connexio-accent transition-colors" />
					</div>
					<div>
						<label className="block text-[11px] font-medium text-connexio-text-secondary mb-1">Host & Port</label>
						<div className="flex gap-2">
							<input type="text" placeholder="hostname or IP" value={host} onChange={(e) => setHost(e.target.value)} onKeyDown={handleKeyDown} className="flex-1 px-3 py-2 text-xs bg-connexio-bg border border-connexio-border rounded-lg text-connexio-text outline-none focus:border-connexio-accent transition-colors" />
							<input type="number" placeholder="22" value={port} onChange={(e) => setPort(Number(e.target.value) || 22)} onKeyDown={handleKeyDown} className="w-20 px-3 py-2 text-xs bg-connexio-bg border border-connexio-border rounded-lg text-connexio-text outline-none focus:border-connexio-accent text-center transition-colors" />
						</div>
					</div>
					<div>
						<label className="block text-[11px] font-medium text-connexio-text-secondary mb-1">Username</label>
						<input type="text" placeholder="root" value={username} onChange={(e) => setUsername(e.target.value)} onKeyDown={handleKeyDown} className="w-full px-3 py-2 text-xs bg-connexio-bg border border-connexio-border rounded-lg text-connexio-text outline-none focus:border-connexio-accent transition-colors" />
					</div>
				</div>
			)}
			...
			{section === "auth" && (
				<div className="space-y-3">
					<div>
						<label className="block text-[11px] font-medium text-connexio-text-secondary mb-1">Authentication method</label>
						<div className="flex gap-1">
							{(["password", "key", "agent"] as const).map((method) => (
								<button key={method} onClick={() => setAuthMethod(method)} className={`flex-1 px-3 py-2 text-xs rounded-lg border transition-colors ${authMethod === method ? "border-connexio-accent bg-connexio-accent/10 text-connexio-accent font-medium" : "border-connexio-border text-connexio-text-muted hover:border-connexio-text-muted"}`} type="button">
					...
					{authMethod === "key" && (
						<div>
							<label className="block text-[11px] font-medium text-connexio-text-secondary mb-1">Private key path</label>
							<div className="flex gap-2">
								<input type="text" placeholder="~/.ssh/id_rsa" value={privateKeyPath} onChange={(e) => setPrivateKeyPath(e.target.value)} onKeyDown={handleKeyDown} className="flex-1 px-3 py-2 text-xs bg-connexio-bg border border-connexio-border rounded-lg text-connexio-text outline-none focus:border-connexio-accent font-mono transition-colors" />
								<button onClick={handleSelectKey} className="px-3 py-2 bg-connexio-bg border border-connexio-border rounded-lg hover:border-connexio-accent/50 transition-colors" type="button" title="Browse for key file"><FolderOpen size={13} className="text-connexio-text-muted" /></button>
							</div>
						</div>
					)}
					{authMethod !== "agent" && (
						<div>
							<label className="block text-[11px] font-medium text-connexio-text-secondary mb-1">{authMethod === "key" ? "Passphrase" : "Password"}</label>
							<input type="password" placeholder={authMethod === "key" ? "Optional — for test & save" : "For test & save"} value={testPassword} onChange={(e) => setTestPassword(e.target.value)} onKeyDown={handleKeyDown} className="w-full px-3 py-2 text-xs bg-connexio-bg border border-connexio-border rounded-lg text-connexio-text outline-none focus:border-connexio-accent transition-colors" />
							<label className="flex items-center gap-1.5 mt-2 text-[11px] text-connexio-text-muted cursor-pointer"><input type="checkbox" checked={rememberSecret} onChange={(e) => setRememberSecret(e.target.checked)} className="w-3.5 h-3.5 rounded" />Save in OS keychain</label>
						</div>
					)}
					{testMessage && (
						<div className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs ${testStatus === "success" ? "text-green-400 bg-green-500/10" : testStatus === "error" ? "text-red-400 bg-red-500/10" : "text-connexio-text-muted bg-connexio-bg"}`}>
							{testStatus === "success" && <Check size={12} />}
							{testMessage}
						</div>
					)}
					{testFingerprint && testHostTrust !== "trusted" && (
						<button onClick={handleTrustHost} className={`w-full text-left text-xs px-3 py-2 rounded-lg border transition-colors ${testHostTrust === "changed" ? "border-red-500/50 text-red-300 bg-red-500/10" : "border-yellow-500/40 text-yellow-300 bg-yellow-500/10"}`} type="button">
			...
			{section === "advanced" && (
				<div className="space-y-3">
					<div>
						<label className="block text-[11px] font-medium text-connexio-text-secondary mb-1">Folder & Keep-alive</label>
						<div className="flex gap-2">
							<input type="text" placeholder="Group folder" value={folder} onChange={(e) => setFolder(e.target.value)} onKeyDown={handleKeyDown} className="flex-1 px-3 py-2 text-xs bg-connexio-bg border border-connexio-border rounded-lg text-connexio-text outline-none focus:border-connexio-accent transition-colors" />
							<div className="flex items-center gap-1">
								<input type="number" value={keepAliveSecs} onChange={(e) => setKeepAliveSecs(Number(e.target.value) || 30)} onKeyDown={handleKeyDown} className="w-16 px-2 py-2 text-xs bg-connexio-bg border border-connexio-border rounded-lg text-connexio-text outline-none focus:border-connexio-accent text-center transition-colors" />
								<span className="text-[10px] text-connexio-text-muted">sec</span>
							</div>
						</div>
					</div>
					<div>
						<label className="block text-[11px] font-medium text-connexio-text-secondary mb-1">Tags</label>
						<input type="text" placeholder="production, web, aws" value={tags} onChange={(e) => setTags(e.target.value)} onKeyDown={handleKeyDown} className="w-full px-3 py-2 text-xs bg-connexio-bg border border-connexio-border rounded-lg text-connexio-text outline-none focus:border-connexio-accent transition-colors" />
					</div>
				</div>
			)}

			{/* Actions */}
			{validationError && <div className="text-xs text-red-400 bg-red-500/10 px-3 py-1.5 rounded-lg">{validationError}</div>}
...
export function SFTPBrowser({ connection, onBack }: { connection: SSHConnection; onBack?: () => void }) {
	...
	useEffect(() => {
		...
		const ref = connection.authMethod === "key" ? connection.passphraseSecretRef : connection.passwordSecretRef;
		if (!ref?.key) {
			setPasswordLoaded(true);
			return;
		}
		window.connexio.ssh.getSecret(ref.key)
			.then((secret) => {
				if (secret) {
					setPassword(secret);
	...
	const openEntry = async (entry: SFTPEntry) => {
		...
		if (entry.size != null && entry.size > MAX_PREVIEW_BYTES) {
			...
			setPreviewContent(`File is too large to preview safely (${formatSize(entry.size)}). Use download support in a later update, or open smaller text files.`);
			return;
		}
		setLoading(true);
		setError("");
	...
	const openInEditor = async () => {
		...
		if (previewContent.startsWith("File is too large") || previewContent.startsWith("Binary file")) return;
	...
	const openEntryInEditor = async (entry: SFTPEntry) => {
		...
		try {
			await ensureSecretSaved();
			const content = await window.connexio.ssh.sftpRead(connection, entry.path, password || undefined);
			if (content.includes("\u0000")) {
	...
	const uploadFile = async () => {
		...
		try {
			...
			await window.connexio.ssh.sftpUpload(connection, selected, joinRemotePath(path, name), password || undefined);
			await loadPath(path);
		} catch (err) {
			setError(String(err));
		} finally {
	...
	return (
		<div className="flex flex-col h-full overflow-hidden bg-connexio-bg text-connexio-text">
			<div className="flex items-center gap-3 px-5 py-4 border-b border-connexio-border bg-connexio-bg-secondary">
				{onBack && <button onClick={onBack} className="text-sm text-connexio-text-muted hover:text-connexio-text" type="button">← Hosts</button>}
				<div className="w-10 h-10 rounded-2xl bg-connexio-accent text-connexio-bg flex items-center justify-center font-bold">{connection.name.slice(0, 2).toUpperCase()}</div>
				<div className="min-w-0 flex-1">
					<div className="text-base font-semibold truncate">{connection.name}</div>
					<div className="text-xs text-connexio-text-muted truncate">{connection.username}@{connection.host}:{connection.port}</div>
				</div>
				<button onClick={uploadFile} disabled={loading} className="px-3 py-2 rounded-xl bg-connexio-bg-tertiary text-sm font-medium disabled:opacity-50" type="button">Upload</button>
				<button onClick={() => loadPath(path)} disabled={loading} className="px-3 py-2 rounded-xl bg-connexio-bg-tertiary text-sm font-medium disabled:opacity-50" type="button">Refresh</button>
			</div>

			<div className="flex-1 min-h-0 flex">
				<aside className="w-64 border-r border-connexio-border bg-connexio-bg-secondary p-3 space-y-3 hidden lg:block">
					<div className="text-xs font-semibold text-connexio-text-muted uppercase tracking-wider">Connection</div>
					{needsPassword && (
						<div className="space-y-2">
							<input
								type="password"
								placeholder={passwordLoaded && password ? "saved secret loaded" : connection.authMethod === "key" ? "passphrase" : "password"}
								value={password}
							...
							<label className="flex items-center gap-2 text-xs text-connexio-text-muted">
								<input type="checkbox" checked={rememberSftpSecret} onChange={(e) => setRememberSftpSecret(e.target.checked)} />
								Save {secretKind} for SFTP/editor
							</label>
							{passwordSaved && <div className="text-xs text-green-600">Saved credential loaded</div>}
						</div>
					)}
					<div className="space-y-1">
						<button onClick={() => loadPath(".")} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-connexio-bg-tertiary text-sm text-left" type="button"><FolderOpen size={15} /> Home</button>
						<button onClick={() => loadPath("/")} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-connexio-bg-tertiary text-sm text-left" type="button"><FolderOpen size={15} /> Root</button>
					</div>
					<div className="pt-3 border-t border-connexio-border space-y-2">
						<div className="text-xs font-semibold text-connexio-text-muted uppercase tracking-wider">New folder</div>
						<input value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") createFolder(); }} placeholder="folder name" className="w-full px-3 py-2 text-sm bg-connexio-bg border border-connexio-border rounded-xl outline-none" />
						<button onClick={createFolder} disabled={loading || !newFolderName.trim()} className="w-full px-3 py-2 rounded-xl bg-connexio-accent text-connexio-bg text-sm disabled:opacity-40" type="button">Create</button>
					</div>
				</aside>
				...
				<main className="flex-1 min-w-0 flex flex-col">
					<div className="px-5 py-3 border-b border-connexio-border bg-connexio-bg-secondary space-y-2">
						<div className="flex gap-2">
							<input value={path} onChange={(e) => setPath(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") loadPath(path); }} className="flex-1 px-3 py-2 text-sm bg-connexio-bg border border-connexio-border rounded-xl outline-none font-mono" />
							<button onClick={() => loadPath(path)} disabled={loading} className="px-4 py-2 rounded-xl bg-connexio-accent text-connexio-bg text-sm disabled:opacity-50" type="button">Go</button>
						</div>
						<div className="flex items-center gap-1 text-xs text-connexio-text-muted">
							<button onClick={() => loadPath("/")} className="hover:text-connexio-text" type="button">/</button>
							{breadcrumbParts.map((part, index) => {
								const next = `/${breadcrumbParts.slice(0, index + 1).join("/")}`;
								return <button key={`${part}-${index}`} onClick={() => loadPath(next)} className="hover:text-connexio-text" type="button">{part} /</button>;
							})}
						</div>
					...
					<div className="flex-1 min-h-0 overflow-auto bg-connexio-bg-secondary relative">
						{loading && (
							<div className="absolute inset-0 z-20 flex items-center justify-center bg-connexio-bg/60 backdrop-blur-sm">
								<div className="rounded-xl border border-connexio-border bg-connexio-bg-secondary px-4 py-3 shadow-2xl min-w-64">
									<div className="text-sm font-medium text-connexio-text">{operationLabel || "Working..."}</div>
									{operationDetail && <div className="mt-1 text-xs text-connexio-text-muted truncate max-w-80">{operationDetail}</div>}
						...
						<table className="w-full text-sm">
							...
							<tbody>
								<tr className="border-b border-connexio-border/50 hover:bg-connexio-bg">
									<td className="px-5 py-2"><button onClick={() => loadPath(parentPath())} className="flex items-center gap-2 text-connexio-text-muted" type="button"><FolderOpen size={15} /> ..</button></td>
									<td />
									<td />
								</tr>
								{entries.map((entry) => (
									<tr key={entry.path} onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); setContextMenu({ x: e.clientX, y: e.clientY, entry }); }} className="group border-b border-connexio-border/50 hover:bg-connexio-bg">
										<td className="px-5 py-2">
											<button onClick={() => openEntry(entry)} className="flex items-center gap-2 min-w-0 text-left w-full" type="button">
												{entry.isDir ? <FolderOpen size={15} className="text-yellow-400 flex-shrink-0" /> : <FileText size={15} className="text-connexio-text-muted flex-shrink-0" />}
												{renamingPath === entry.path ? <input value={renameValue} onChange={(e) => setRenameValue(e.target.value)} onClick={(e) => e.stopPropagation()} onKeyDown={(e) => { if (e.key === "Enter") commitRename(entry); if (e.key === "Escape") setRenamingPath(null); }} className="flex-1 px-2 py-1 bg-connexio-bg text-connexio-text border border-connexio-accent/50 rounded outline-none focus:border-connexio-accent" autoFocus /> : <span className="truncate">{entry.name}</span>}
											</button>
										</td>
										<td className="px-3 py-2 text-right text-connexio-text-muted text-xs">{entry.isDir ? "--" : formatSize(entry.size)}</td>
										<td className="px-3 py-2">
											<div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
												{!entry.isDir && <button onClick={() => openEntryInEditor(entry)} className="p-1 rounded hover:bg-connexio-bg-tertiary" type="button" title="Open in Connexio editor"><Pencil size={13} /></button>}
								<button onClick={() => renamingPath === entry.path ? commitRename(entry) : startRename(entry)} className="p-1 rounded hover:bg-connexio-bg-tertiary" type="button" title="Rename"><span className="text-[11px]">Aa</span></button>
												{!entry.isDir && <button onClick={() => downloadEntry(entry)} className="p-1 rounded hover:bg-connexio-bg-tertiary" type="button" title="Download"><FileText size={13} /></button>}
								<button onClick={() => deleteEntry(entry)} className="p-1 rounded hover:bg-red-500/10" type="button"><Trash2 size={13} className="text-red-400" /></button>
											</div>
										</td>
									</tr>
								))}
						...
						{loading && <div className="p-4 text-sm text-connexio-text-muted">{operationLabel || "Loading..."}</div>}
					</div>
				</main>
				...
				{contextMenu && (
					<SSHManagerContextMenu
						...
						items={[
							{ label: contextMenu.entry.isDir ? "Open Folder" : "Preview", onClick: () => openEntry(contextMenu.entry) },
							...(contextMenu.entry.isDir ? [] : [
								{ label: "Open in Connexio Editor", onClick: () => openEntryInEditor(contextMenu.entry) },
				...
				{previewPath && (
					<aside className="w-[38%] max-w-[560px] min-w-[320px] border-l border-connexio-border bg-connexio-bg-secondary hidden xl:flex flex-col">
						<div className="flex items-center gap-2 px-4 py-3 border-b border-connexio-border">
							...
							<span className="flex-1 min-w-0 truncate text-sm font-mono text-connexio-text-secondary">{previewPath}</span>
							<button onClick={openInEditor} className="px-2 py-1 text-xs rounded-lg bg-connexio-accent/10 text-connexio-accent" type="button">Edit</button>
							<button onClick={() => { setPreviewPath(null); setPreviewContent(""); }} className="p-1 rounded hover:bg-connexio-bg-tertiary" type="button"><X size={14} /></button>
						</div>
						<pre className="flex-1 overflow-auto p-4 text-xs text-connexio-text bg-connexio-bg font-mono whitespace-pre-wrap">{previewContent}</pre>
					</aside>
				)}
			</div>
			...
			{hosts.map((host) => (
				<div key={`${host.host}:${host.port}`} className="p-2 rounded border border-connexio-border bg-connexio-bg-secondary space-y-1">
					<div className="flex items-center gap-2">
						...
						<button onClick={() => forget(host)} className="p-0.5 rounded hover:bg-red-500/100/20" type="button" title="Forget"><Trash2 size={10} className="text-red-400" /></button>
					</div>
...
function SSHHostCard({
	...
	const authLabel = connection.authMethod === "key" ? "Key" : connection.authMethod === "agent" ? "Agent" : "Password";
	const authColor = connection.authMethod === "key" ? "text-yellow-400" : connection.authMethod === "agent" ? "text-green-400" : "text-connexio-text-muted";
	return (
		<div onContextMenu={onContextMenu} className="group min-h-[76px] rounded-2xl bg-connexio-bg-secondary border border-connexio-border hover:border-connexio-accent/40 transition-all px-4 py-3 flex items-center gap-3">
			<button onClick={onConnect} className="flex items-center gap-3 flex-1 min-w-0 text-left" type="button">
				<div className={`${isConnecting ? "animate-pulse bg-connexio-accent" : "bg-connexio-accent"} w-12 h-12 rounded-2xl flex items-center justify-center text-connexio-bg font-bold text-sm flex-shrink-0`}>
					{iconText}
				</div>
				<div className="min-w-0">
					<div className="text-sm font-semibold text-connexio-text truncate">{connection.name}</div>
					<div className="text-xs text-connexio-text-muted truncate">{connection.username}@{connection.host}{connection.port !== 22 ? `:${connection.port}` : ""}</div>
					<div className="flex items-center gap-2 mt-0.5">
						<span className={`text-[10px] ${authColor}`}>{authLabel}</span>
						{connection.tags && connection.tags.length > 0 && (
							<div className="flex gap-1 overflow-hidden">
								{connection.tags.slice(0, 2).map((tag) => <span key={tag} className="px-1.5 py-0.5 rounded bg-connexio-bg-tertiary text-[10px] text-connexio-text-muted">{tag}</span>)}
							</div>
						)}
					</div>
				</div>
			...
			<div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
				<button onClick={onSftp} className="p-1.5 rounded-lg hover:bg-connexio-bg-tertiary" type="button" title="Browse files (SFTP)"><FolderOpen size={14} className="text-connexio-text-muted" /></button>
				<button onClick={onEdit} className="p-1.5 rounded-lg hover:bg-connexio-bg-tertiary" type="button" title="Edit connection"><Pencil size={14} className="text-connexio-text-muted" /></button>
				<button onClick={onDelete} className="p-1.5 rounded-lg hover:bg-red-500/10" type="button" title="Delete connection"><Trash2 size={14} className="text-red-400" /></button>
			</div>
		</div>
	);
}
