		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Settings">
			<div className="bg-connexio-bg-secondary border border-connexio-border rounded-lg w-[600px] max-h-[500px] shadow-2xl flex flex-col">
				...
				<div className="flex flex-1 overflow-hidden">
					{/* Sidebar tabs */}
					<div className="w-40 border-r border-connexio-border py-2 px-2 space-y-0.5" role="tablist" aria-label="Settings categories">
						{tabs.map((tab) => (
							<button
								key={tab.id}
				...
				<div className="flex gap-2">
					{(["bar", "block", "underline"] as const).map((style) => (
						<button
							...
							type="button"
							aria-label={`${style} cursor style`}
							aria-pressed={settings.cursorStyle === style}
						>
							{style}
						</button>
					))}
