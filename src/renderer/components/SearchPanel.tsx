		...
		<div className="flex flex-col h-full">
			...
			<div className="p-2 border-b border-connexio-border">
				<div className="flex items-center gap-1 bg-connexio-bg-tertiary rounded px-2 py-1">
					...
					/>
					{query && (
						<button onClick={handleClear} className="p-0.5 rounded hover:bg-connexio-bg-secondary" type="button" aria-label="Clear search">
						...
						{Object.entries(grouped).map(([filePath, fileResults]) => (
							<div key={filePath} className="mb-1">
								...
								<button
									...
									type="button"
									aria-label={`Open file ${fileName(filePath)}`}
								>
								...
								{fileResults.map((r) => (
									<button
										...
										type="button"
										aria-label={`Go to line ${r.lineNumber} in ${fileName(r.filePath)}`}
									>
